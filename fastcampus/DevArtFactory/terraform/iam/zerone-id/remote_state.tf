...
data "terraform_remote_state" "secretsmanager" {
  backend = "s3"
  #   config  = merge(var.remote_state.kms.jindol.apne2)
  config = merge(var.remote_state.secretsmanager.jindol.apne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
}
...
data "terraform_remote_state" "codedeploy" {
  backend = "s3"
  #   config  = merge(var.remote_state.codedeploy.jindol.apne2)
  config = merge(var.remote_state.codedeploy.jindol.apne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
}
