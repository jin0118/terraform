...
data "terraform_remote_state" "security_group" {
  backend = "s3"
  # config = merge(var.remote_state.security_group.jindol.tmcdapne2)
  config = merge(var.remote_state.security_group.jindol.tmcdapne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
}
